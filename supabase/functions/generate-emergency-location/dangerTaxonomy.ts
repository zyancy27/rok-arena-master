// (mirrors src/engine/scenarioBrain/dangerTaxonomy.ts)
...
const EXOTIC_FAMILIES: DangerFamily[] = ['supernatural-paranormal','scifi-advanced-systems'];
const SPECULATIVE_RARITY_FAMILIES: DangerFamily[] = ['technological-malfunction'];
...
export function inferGroundedness(b: EmergencyBlueprint & DangerTaxonomyFields): Groundedness {
  ...
  if (EXOTIC_FAMILIES.includes(fam)) return b.rarity === 'advanced' ? 'speculative' : 'exotic';
  if (SPECULATIVE_RARITY_FAMILIES.includes(fam) && b.rarity === 'advanced') return 'speculative';
  return 'grounded';
}
