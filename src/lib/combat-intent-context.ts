...
const ATTACK_VERBS = /\b(attack|strike|stab|slash|shoot|punch|kick|smash|lunge|swing|blast|pierce|impale|cleave|slice|thrust|grapple|tackle|choke|cut(?:\s+at)?|threaten)\b/i;
const RANGED_ATTACK_PATTERNS = /\b(throw|hurl|launch|fire)\b(?:[\w'-]+\s+){0,4}\b(at|into|toward|towards|against)\b/i;
...
const DIRECTED_PREPOSITION = /\b(at|into|toward|towards|against)\s+(?:the\s+|a\s+|an\s+|my\s+|his\s+|her\s+|their\s+)?[a-z][\w'-]*(?:\s+[a-z][\w'-]*){0,2}\b/i;
